export const querys = {
  ...
  getAllActivos: "SELECT EQC_id,EQC_numEquipo,EQC_activoFijo,EQUIP_id,EQUIP_modelo,EQUIP_descripcion,MARCA_id,MARCA_descripcion,EQC_MAP_ciudad,EQC_MAP_provincia,EQC_MAP_address,USU_id,USU_nombre,EQC_fecha_ing,EQC_codTag,ATENC_id,ATENC_descripcion,EQC_nombreCliente,VEND_id,VEND_nombres,EQC_direccionCliente,EQC_NegocioCliente,CANAL_id,CANAL_descripcion,EQC_cambio,EQC_estadoEquipo,EQC_observacion,SUBEST_id,SUBEST_descripcion,UBIC_id,UBIC_ciudad,EQC_provincia,EQC_numCuerpos,SUBCAN_id,SUBCAN_descripcion,SEG_id,SEG_descripcion,EQC_CodigoSap,EQC_RucCedula,SUP_id,SUP_descripcion,SUBSEG_id,SUBSEG_descripcion,EQC_local,EQC_coordenadas,EQC_contacto, EQC_correo,TE_id,TE_descripcion,EQC_CodigoSapActivo,EQC_fechaInstalacion,INST_id,INST_descripcion,COND_id,COND_descripcion,EQC_telefonoCliente FROM EQUIPO_COMPLETO AS E INNER JOIN EQUIPO ON EQC_EQUIP_id = EQUIP_id INNER JOIN ATENCION ON EQC_ATENC_id = ATENC_id INNER JOIN MARCA ON EQC_MARCA_id = MARCA_id INNER JOIN USUARIOS ON EQC_USU_ing = USU_id INNER JOIN UBICACION ON EQC_UBIC_id = UBIC_id INNER JOIN VENDEDOR ON EQC_VEND_id = VEND_id INNER JOIN CANAL ON EQC_CANAL_id = CANAL_id INNER JOIN SUBESTADO ON EQC_SUBEST_id = SUBEST_id INNER JOIN SUBCANAL ON EQC_SUBCAN_id = SUBCAN_id INNER JOIN SEGMENTO ON EQC_SEG_id = SEG_id INNER JOIN SUPERVISOR ON EQC_SUP_id = SUP_id INNER JOIN SUBSEGMENTACION ON EQC_SUBSEG_id = SUBSEG_id INNER JOIN TIPO_EQUIPO ON EQC_TE_id = TE_id INNER JOIN INSTALACION ON EQC_INST_id = INST_id INNER JOIN CONDICION ON EQC_COND_id = COND_id",
