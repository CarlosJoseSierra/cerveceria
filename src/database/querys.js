export const querys = {
  ...
  getAllActivos: "SELECT EQC_id,EQC_serie,EQC_placa,EQC_EQUIP_id,EQC_MAP_ciudad,EQC_MAP_provincia,EQC_MAP_address,EQC_USU_ing,EQC_fecha_ing,EQC_codTag,EQC_LOGO_id,EQC_nombreCliente,EQC_identificacionCliente,EQC_direccionCliente,EQC_NegocioCliente,EQC_telefonoCliente,EQC_cambio, EQUIP_modelo,EQUIP_marca, EQUIP_descripcion,LOGO_nombre FROM EQUIPO_COMPLETO AS E inner join EQUIPO ON EQC_EQUIP_id = EQUIP_id inner join LOGO ON EQC_LOGO_id = LOGO_id WHERE EQC_estadoEquipo = 1",
  getActivoByUsuario: "SELECT EQC_id,EQC_serie,EQC_placa,EQC_EQUIP_id,EQC_MAP_ciudad,EQC_MAP_provincia,EQC_MAP_address,EQC_USU_ing,EQC_fecha_ing,EQC_codTag, EQC_LOGO_id,EQC_nombreCliente,EQC_identificacionCliente,EQC_direccionCliente,EQC_NegocioCliente,EQC_telefonoCliente, EQUIP_modelo,EQUIP_marca, EQUIP_descripcion,LOGO_nombre FROM EQUIPO_COMPLETO AS E inner join EQUIPO ON EQC_EQUIP_id = EQUIP_id inner join LOGO ON EQC_LOGO_id = LOGO_id WHERE EQC_estadoEquipo = 1 and EQC_USU_ing = @idUsuario",  
  getActivoById: "SELECT * FROM EQUIPO_COMPLETO Where EQC_id = @Id",
  getActivoByTag: "SELECT EQC_id,EQC_serie,EQC_placa,EQC_EQUIP_id,EQUIP_modelo,EQUIP_marca,EQUIP_descripcion,EQUIP_compresor,EQUIP_puertas,EQC_MAP_ciudad,EQC_MAP_provincia,EQC_MAP_address,EQC_fecha_ing,EQC_codTag,EQC_LOGO_id, LOGO_nombre,EQC_nombreCliente,EQC_identificacionCliente,EQC_NegocioCliente,EQC_telefonoCliente,EQC_direccionCliente,EQC_estadoEquipo FROM EQUIPO_COMPLETO AS E inner join EQUIPO ON EQC_EQUIP_id = EQUIP_id inner join LOGO ON EQC_LOGO_id = LOGO_id Where EQC_codTag = @EQC_codTag",
  addNewActivo:
    "INSERT INTO EQUIPO_COMPLETO(EQC_serie,EQC_placa,EQC_EQUIP_id,EQC_MAP_ciudad,EQC_MAP_provincia,EQC_MAP_address,EQC_USU_ing,EQC_fecha_ing,EQC_codTag,EQC_LOGO_id,EQC_nombreCliente,EQC_identificacionCliente,EQC_direccionCliente,EQC_NegocioCliente,EQC_telefonoCliente,EQC_estadoEquipo,EQC_cambio) VALUES(@EQC_serie,@EQC_placa,@EQC_EQUIP_id,@EQC_MAP_ciudad,@EQC_MAP_provincia,@EQC_MAP_address,@EQC_USU_ing,GETDATE(),@EQC_codTag,@EQC_LOGO_id,@EQC_nombreCliente,@EQC_identificacionCliente,@EQC_direccionCliente,@EQC_NegocioCliente,@EQC_telefonoCliente,1,@EQC_cambio)",
  deleteActivo: "DELETE FROM EQUIPO_COMPLETO WHERE EQC_id= @Id",
  getTotalActivos: "SELECT COUNT(*) FROM EQUIPO_COMPLETO",
  updateActivoById:
    "UPDATE EQUIPO_COMPLETO SET EQC_serie = @EQC_serie,EQC_placa = @EQC_placa,EQC_EQUIP_id = @EQC_EQUIP_id,EQC_MAP_ciudad = @EQC_MAP_ciudad,EQC_MAP_provincia = @EQC_MAP_provincia,EQC_MAP_address = @EQC_MAP_address,EQC_USU_ing = @EQC_USU_ing,EQC_fecha_ing = GETDATE(),EQC_codTag = @EQC_codTag,EQC_LOGO_id = @EQC_LOGO_id,EQC_nombreCliente = @EQC_nombreCliente,EQC_identificacionCliente = @EQC_identificacionCliente,EQC_direccionCliente = @EQC_direccionCliente,EQC_NegocioCliente = @EQC_NegocioCliente,EQC_telefonoCliente = @EQC_telefonoCliente,EQC_cambio = @EQC_cambio,EQC_estadoEquipo = 1 WHERE EQC_id = @Id",
  ...
  getHistorialEquipos: "SELECT HIST_id,HIST_EQC_id,HIST_placa,HIST_serie,HIST_codTag,CONVERT(varchar,HIST_FECHA_ing,3) as HIST_FECHA_ing,HIST_USU_ing,HIST_MAP_ciudad,HIST_MAP_provincia,HIST_MAP_address,HIST_nombreCliente,HIST_identificacionCliente,HIST_direccionCliente,HIST_NegocioCliente,HIST_telefonoCliente,USU_nombre AS HIST_USU_nombre FROM HISTORIAL_MOVIMIENTOEQUIPO INNER JOIN USUARIOS ON HIST_USU_ing = USU_id WHERE HIST_EQC_id = @Id",
  ...
  getDataTopFiveVendedores:"SELECT TOP 5 T.USU_nombre AS DESCRIPCION,COUNT(DISTINCT(T.EQC_id))AS CONTEO FROM(SELECT EQC_USU_ing,EQC_id, USU_nombre FROM EQUIPO_COMPLETO INNER JOIN USUARIOS ON EQC_USU_ing = USU_id WHERE EQC_estadoEquipo = 1)  T GROUP BY T.USU_nombre ORDER BY CONTEO DESC",
